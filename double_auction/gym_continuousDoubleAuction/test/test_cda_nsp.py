...
class MyCallbacks(DefaultCallbacks):
    def on_episode_start(self, worker, base_env: BaseEnv,
                         ...
                         episode, **kwargs):
        """
        info["episode"] is an Episode object.
    ...
    def on_sample_end(self, worker, samples: SampleBatch,
                      **kwargs):
        """
        arg: {"samples": .., "worker": ...}
...
def go_train(config):
    # Create PPO algorithm using PPOConfig
    ppo_config = PPOConfig().update_from_dict(config)
    trainer = ppo_config.build(env="continuousDoubleAuction-v0")
