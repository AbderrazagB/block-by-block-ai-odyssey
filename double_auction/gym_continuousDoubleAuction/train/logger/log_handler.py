...
def log_g_store(log_g_store_dir, num_agents, experiment_id):
    ...
    for agt_key, _ in store.items():
        ...
        tmp_dict[agt_key]["step"]["obs"] = store[str(agt_key)]["step"]["obs"][-1] if store[str(agt_key)]["step"]["obs"] else []
        tmp_dict[agt_key]["step"]["act"] = store[agt_key]["step"]["act"][-1] if store[agt_key]["step"]["act"] else []
        tmp_dict[agt_key]["step"]["reward"] = store[agt_key]["step"]["reward"][-1] if store[agt_key]["step"]["reward"] else []
        tmp_dict[agt_key]["step"]["NAV"] = store[agt_key]["step"]["NAV"][-1] if store[agt_key]["step"]["NAV"] else []
        tmp_dict[agt_key]["step"]["num_trades"] = store[agt_key]["step"]["num_trades"][-1] if store[agt_key]["step"]["num_trades"] else []
