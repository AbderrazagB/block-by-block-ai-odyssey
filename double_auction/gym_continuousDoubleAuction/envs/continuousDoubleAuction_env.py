...
class continuousDoubleAuctionEnv(MultiAgentEnv, Exchg_Helper):

    metadata = {'render_modes': ['human'], 'render_fps': 30}

    def __init__(self, num_of_agents=2, init_cash=0, tick_size=1, tape_display_length=10, max_step=100, render_mode=None):
        MultiAgentEnv.__init__(self)
        Exchg_Helper.__init__(self, init_cash, tick_size, tape_display_length)
        ...
        obs_row = 4
        obs_col = 10
        self.observation_space = spaces.Box(low=neg_inf, high=inf, shape=(obs_row,obs_col), dtype=np.float32)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        return self.reset_traders_agg_LOB(), self.infos
    ...
    def step(self, actions):
        ...
        truncateds = {agent_id: False for agent_id in self.next_states}
        truncateds["__all__"] = False

        return self.next_states, self.rewards, terminateds, truncateds, self.infos
